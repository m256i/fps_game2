...
#define CREATE_LINKED_LIST_TYPE(TYPE)                                                                                  \
  ...
  static inline l_list_##TYPE l_list_##TYPE##_create(l_list_copy_fn cp, l_list_destructor_fn dt) {                     \
    return (l_list_##TYPE){.head = NULL, .tail = NULL, .size = 0, .copier = cp, .dtor = dt};                           \
  }                                                                                                                    \
                                                                                                                       \
  /** Initialize to empty list **/                                                                                     \
  static inline l_list_##TYPE l_list_##TYPE##_create_() {                                                              \
    return (l_list_##TYPE){.head = NULL, .tail = NULL, .size = 0, .copier = NULL, .dtor = NULL};                       \
  }                                                                                                                    \
                                                                                                                       \
  ...
  static inline void l_list_##TYPE##_free(l_list_##TYPE *l) {                                                          \
    ...
    l->size = 0;                                                                                                       \
                                                                                                                       ...
                                                                                                                       \
  static inline int l_list_##TYPE##_empty(const l_list_##TYPE *l) { return l->size == 0; }                             \
  static inline size_t l_list_##TYPE##_size(const l_list_##TYPE *l) { return l->size; }                                \
                                                                                                                       \
  /** Internal: allocate node, copy value **/                                                                          \
  static inline TYPE##_node *l_list_##TYPE##_make_node(l_list_##TYPE *l, const TYPE *v) {                              \
    ...
    return node;                                                                                                       \
                                                                                                                       ...
                                                                                                                       \
  /** Push at back **/                                                                                                 \
  static inline void l_list_##TYPE##_push_back(l_list_##TYPE *l, const TYPE *v) {                                      \
    ...
    l->size++;                                                                                                         \
                                                                                                                       ...
                                                                                                                       \
  /** Push at front **/                                                                                                \
  static inline void l_list_##TYPE##_push_front(l_list_##TYPE *l, const TYPE *v) {                                     \
    TYPE##_node *n = l_list_##TYPE##_make_node(l, v);                                                                  \
    n->next = l->head;                                                                                                 \
  ...
  static inline int l_list_##TYPE##_insert_after(l_list_##TYPE *l, TYPE##_node *pos, const TYPE *v) {                  \
    if (!pos) return -1;                                                                                               \
    TYPE##_node *n = l_list_##TYPE##_make_node(l, v);                                                                  \
    n->next = pos->next;                                                                                               \
    n->prev = pos;                                                                                                     \
  ...
  static inline int l_list_##TYPE##_insert_before(l_list_##TYPE *l, TYPE##_node *pos, const TYPE *v) {                 \
    if (!pos) return -1;                                                                                               \
    TYPE##_node *n = l_list_##TYPE##_make_node(l, v);                                                                  \
    n->prev = pos->prev;                                                                                               \
    n->next = pos;                                                                                                     \
  ...
  static inline int l_list_##TYPE##_remove_node(l_list_##TYPE *l, TYPE##_node *n) {                                    \
    if (!n) return -1;                                                                                                 \
  ...
  static inline TYPE##_node *l_list_##TYPE##_find(const l_list_##TYPE *l, const TYPE *v) {                             \
  ...
  static inline void l_list_##TYPE##_splice(l_list_##TYPE *l, TYPE##_node *pos, l_list_##TYPE *other) {                \
  ...
  static inline void l_list_##TYPE##_for_each(const l_list_##TYPE *l, void (*fn)(TYPE * elem, void *user_data),        \
                                              void *user_data) {                                                       \
    for (TYPE##_node *it = l->head; it; it = it->next)                                                                 \
  ...
  static inline TYPE##_node *l_list_##TYPE##_find_if(                                                                  \
      const l_list_##TYPE *l, int (*predicate)(const TYPE *elem, void *user_data), void *user_data) {                  \
    for (TYPE##_node *it = l->head; it; it = it->next)                                                                 \
  ...
  /* non-zero. */                                                                                                      \
  static inline void l_list_##TYPE##_remove_if(l_list_##TYPE *l, int (*predicate)(const TYPE *elem, void *user_data),  \
                                               void *user_data) {                                                      \
    TYPE##_node *it = l->head;                                                                                         \
    while (it) {                                                                                                       \
      TYPE##_node *next = it->next;                                                                                    \
      if (predicate(&it->value, user_data)) { l_list_##TYPE##_remove_node(l, it); }                                    \
      it = next;                                                                                                       \
    }                                                                                                                  \
  }                                                                                                                    \
  /**                                                                                                                  \
   * Insert value *v* at position idx (0 == before head, idx == size == at end).                                       \
   * Returns 0 on success, -1 if idx > size.                                                                           \
   */                                                                                                                  \
  static inline int l_list_##TYPE##_insert_at(l_list_##TYPE *l, size_t idx, const TYPE *v) {                           \
   ...
   * optionally writing its value into *out. Returns 0 on success, -1 if empty or idx >= size.                         \
   */                                                                                                                  \
  static inline int l_list_##TYPE##_remove_at(l_list_##TYPE *l, size_t idx, TYPE *out) {                               \
    if (idx >= l->size) return -1;                                                                                     \
  ...
  static inline TYPE##_node *l_list_##TYPE##_node_at(l_list_##TYPE *l, size_t idx) {                                   \
    TYPE##_node *cur = l->head;                                                                                        \
    while (cur && idx--)                                                                                               \
      cur = cur->next;                                                                                                 \
    return cur;                                                                                                        \
   ...
   */                                                                                                                  \
  static inline const TYPE *l_list_##TYPE##_at_const(const l_list_##TYPE *l, size_t idx) {                             \
    /* cast away const to reuse node_at */                                                                             \
    return (const TYPE *)(l_list_##TYPE##_node_at((l_list_##TYPE *)l, idx)                                             \
   ...
   */                                                                                                                  \
  static inline int l_list_##TYPE##_get_at(const l_list_##TYPE *l, size_t idx, TYPE *out) {                            \
