...
#define CREATE_SPSC_U64_IMPL(N)                                                                                        \
  ...
  static inline bool spsc_u64_##N##_enqueue(spsc_u64_##N##_ring_t *q, uint64_t v) {                                    \
    ...
    atomic_store_explicit(&q->idx.tail, (tail + 1) & (N - 1), memory_order_release);                                   \
    return true;                                                                                                       \
  }                                                                                                                    \
  static inline bool spsc_u64_##N##_dequeue(spsc_u64_##N##_ring_t *q, uint64_t *out) {                                 \
    ...
    *out = q->buffer[head];                                                                                            \
    atomic_store_explicit(&q->idx.head, (head + 1) & (N - 1), memory_order_release);                                   \
    return true;                                                                                                       \
  }                                                                                                                    \
  static inline bool spsc_u64_##N##_peek(spsc_u64_##N##_ring_t *q, uint64_t *out) {                                    \
    size_t tail = atomic_load_explicit(&q->idx.tail, memory_order_acquire);                                            \
    size_t head = atomic_load_explicit(&q->idx.head, memory_order_relaxed);                                            \
    if (head == tail) return false;                                                                                    \
    *out = q->buffer[head];                                                                                            \
