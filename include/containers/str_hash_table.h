...
u0 str_hash_table_initialize(str_hash_table *const _table, usize _obj_size, usize _obj_count) {
  ...
  for (usize i = 0; i != _table->str_hasher.bucket_count; i++) {
    if (_table->str_hasher.data[i].used && _table->str_hasher.data[i].key) {
      printf("index: %u hash: %u key: %s value_first_byte: %u,\n", _table->str_hasher.data[i].id,
             _table->str_hasher.data[i].hash, _table->str_hasher.data[i].key, *(u8 *)_table->buckets[i].data);
    }
  }
  puts("}");
}

u32 str_hash_table_insert(str_hash_table *const _table, const char *_key, u0 *_obj) {
  assert(_table->initialized);
  ...
  if (idx == UINT32_MAX) {
    ...
    if (_table->str_hasher.bucket_count != _table->obj_array_len) {
      ...
      _table->obj_array_len = _table->str_hasher.bucket_count;
      _table->buckets = zrealloc(_table->buckets, old_size * sizeof(str_hash_table_slot),
...
u0 str_hash_table_erase(str_hash_table *const _table, const char *_key) {
  ...
  if (_table->buckets[idx].data) { free(_table->buckets[idx].data); }
...
u0 *str_hash_table_at_index(str_hash_table *const _table, u32 _index) {
  ...
  /* slot empty */
  if (!_table->buckets[_index].used) { return NULL; }
  /* slot not empty */
...
u0 str_hash_table_destroy(str_hash_table *const _table) {
  ...
  for (usize i = 0; i != _table->obj_array_len; i++) {
    if (_table->buckets[i].data) { free(_table->buckets[i].data); }
  }
  free(_table->buckets);
  _table->buckets = NULL;
