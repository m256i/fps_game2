...
/* 
this exists so we can access the u32 creation_info_type of every possible info type 
without knowing which one it is beforehand
*/
typedef struct {
  /* this NEEDS to be the first element */
  u32   creation_info_type;
...
// clang-format on
/* deep copy function for gl_resource_data since clients might pass in temporary structs and arrays */
static gl_resource_data create_persistent_resource_data(const gl_resource_data *const _temp) {
  gl_resource_data out;
  GAME_LOGF("doig deep copy on resource %s", _temp->resource_name);

  /* copy the name into a persistent buff */
  usize temp_name_len = strnlen_s(_temp->resource_name, MAXIMUM_RESOURCE_NAME_STRLEN);
  GAME_LOGF("name strnlen: %zu", temp_name_len);

  out.resource_name = malloc(temp_name_len + 1);
  strncpy_s(out.resource_name, temp_name_len + 1, _temp->resource_name, MAXIMUM_RESOURCE_NAME_STRLEN);
  ...
  case RESOURCE_CREATION_INFO_TYPE_VERTEX_BUFFER: {
    usize attributes_byte_size =
        sizeof *_temp->desc.vertex_buffer.vertex_attributes * _temp->desc.vertex_buffer.num_attributes;
    ...
    memcpy(out.desc.vertex_buffer.vertex_attributes, _temp->desc.vertex_buffer.vertex_attributes, attributes_byte_size);
    break;
  }
  case RESOURCE_CREATION_INFO_TYPE_FRAME_BUFFER: {
    /* deep copy the attachements */
    usize attachements_byte_size =
        sizeof *_temp->desc.frame_buffer.attachements * _temp->desc.frame_buffer.num_attachements;
    memcpy(&out.desc, &_temp->desc, sizeof out.desc);
    out.desc.frame_buffer.attachements = malloc(attachements_byte_size);
    memcpy(out.desc.frame_buffer.attachements, _temp->desc.frame_buffer.attachements, attachements_byte_size);
    break;
  }
  ...
  default: {
    GAME_CRITICALF("unknown creation info passed to create_persistent_resource_data");
    exit(1);
    break;
  }
  }
...
/* cancer */
static bool resource_data_eq(const gl_resource_data *const _r0, const gl_resource_data *const _r1) {
  bool same_info_type = _r0->desc.dummy.creation_info_type == _r1->desc.dummy.creation_info_type;
  ...
  case RESOURCE_CREATION_INFO_TYPE_FRAME_BUFFER: {
    ...
    if (a->width == b->width && a->height == b->height && a->format == b->format &&
        a->num_attachements == b->num_attachements) {
      same_desc = true;
  ...
  case RESOURCE_CREATION_INFO_TYPE_VERTEX_BUFFER: {
    ...
    if (a->size == b->size && a->buffer_usage == b->buffer_usage && a->num_attributes == b->num_attributes) {
      same_desc = true;
      for (usize i = 0; i < a->num_attributes; ++i) {
        ...
        if (va->attribute_type != vb->attribute_type || va->attribute_count != vb->attribute_count ||
  ...
  GAME_LOGF("resource_data_eq() same info: %s same name: %s same desc: %s", same_info_type ? "true" : "false",
            same_name ? "true" : "false", same_desc ? "true" : "false");
...
static u0 request_resource(gl_resource_data *resource_data, gl_resource_handle *_handle) {
  ...
  /* have we already created that resource (client handle not null) for that client */
  if (*_handle != NULL) {
#ifdef GAME_DEBUG
    ...
    if (resource_data_eq(&resource_slot->resource_data, resource_data)) {
      ...
      GAME_CRITICALF("tried to obtain resource '%s' with conflicting creation info", resource_data->resource_name);
      exit(1);
    }
#else
    ...
    GAME_LOGF("new client called request_resource on existing resource: %s", resource_data->resource_name);
    /* handle NULL but resource exists already */
    *_handle = malloc(sizeof **_handle);
    (*_handle)->hashed_resource_index = str_hash_table_get_index(table, resource_data->resource_name);
    resource_table_slot *resource_slot = str_hash_table_at_index(table, (*_handle)->hashed_resource_index);
  ...
  *_handle = malloc(sizeof **_handle);
  (*_handle)->hashed_resource_index =
      str_hash_table_insert(table, resource_data->resource_name,
                            &(resource_table_slot){
...
static u0 destroy_resource(gl_resource_data *resource_data, gl_resource_handle *_handle) {
  static str_hash_table *const table = &gl_resource_manager.table;
