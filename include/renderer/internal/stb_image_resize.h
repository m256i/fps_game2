/* stb_image_resize - v0.90 - public domain image resizing
   ...
   ADDITIONAL DOCUMENTATION
      ...
      PROGRESS
         ...
         The parameter val is a float which goes from 0 to 1 as progress is
   made.
...
typedef enum {
  ...
  STBIR_FILTER_BOX = 1, // A trapezoid w/1-pixel wide ramps, same result as box
                        // for integer scale ratios
  STBIR_FILTER_TRIANGLE =
    2, // On upsampling, produces same results as bilinear texture filtering
  STBIR_FILTER_CUBICBSPLINE = 3, // The cubic b-spline (aka Mitchell-Netrevalli
...
typedef unsigned char
  stbir__validate_uint32[sizeof(stbir_uint32) == 4 ? 1 : -1];
...
typedef struct {
  ...
  int ring_buffer_length_bytes; // The length of an individual entry in the ring
                                // buffer. The total number of ring buffers is
...
static float stbir__srgb_to_linear(float f) {
  ...
  else return (float)pow((f + 0.055f) / 1.055f, 2.4f);
}

static float stbir__linear_to_srgb(float f) {
  if (f <= 0.0031308f) return f * 12.92f;
  else return 1.055f * (float)pow(f, 1 / 2.4f) - 0.055f;
}
...
static float stbir__filter_cubic(float x, float s) {
  ...
  else if (x < 2.0f) return (8 + x * (-12 + x * (6 - x))) / 6;
...
static float stbir__filter_catmullrom(float x, float s) {
  ...
  else if (x < 2.0f) return 2 - x * (4 + x * (0.5f * x - 2.5f));
...
static float stbir__filter_mitchell(float x, float s) {
  ...
  else if (x < 2.0f) return (32 + x * (-60 + x * (36 - 7 * x))) / 18;
...
static int stbir__get_coefficient_width(stbir_filter filter, float scale) {
  if (stbir__use_upsampling(scale))
    return (int)ceil(stbir__filter_info_table[filter].support(1 / scale) * 2);
  else return (int)ceil(stbir__filter_info_table[filter].support(scale) * 2);
}

static int stbir__get_contributors(
  float        scale,
  stbir_filter filter,
  int          input_size,
  int          output_size
) {
  if (stbir__use_upsampling(scale)) return output_size;
  else return (input_size + stbir__get_filter_pixel_margin(filter, scale) * 2);
}

static int stbir__get_total_horizontal_coefficients(stbir__info *info) {
  return info->horizontal_num_contributors * stbir__get_coefficient_width(
...
static int stbir__get_total_vertical_coefficients(stbir__info *info) {
  return info->vertical_num_contributors * stbir__get_coefficient_width(
                                             info->vertical_filter,
                                             info->vertical_scale
                                           );
}

static stbir__contributors *
stbir__get_contributor(stbir__contributors *contributors, int n) {
...
// it there too.
static float *stbir__get_coefficient(
  float       *coefficients,
  stbir_filter filter,
  float        scale,
  int          n,
...
static int stbir__edge_wrap_slow(stbir_edge edge, int n, int max) {
  ...
  case STBIR_EDGE_ZERO:
    return 0; // we'll decode the wrong pixel here, and then overwrite with 0s
              // later
...
static void stbir__calculate_coefficients_upsample(
  ...
  for (i = 0; i <= in_last_pixel - in_first_pixel; i++) {
    ...
    coefficient_group[i]  = stbir__filter_info_table[filter].kernel(
      in_center_of_out - in_pixel_center,
      1 / scale
    );
  ...
  STBIR__DEBUG_ASSERT(
    stbir__filter_info_table[filter].kernel(
...
static void stbir__normalize_downsample_coefficients(
  ...
  for (j = 0; j < num_contributors; j++) {
    ...
    skip = 0;
    while (
      *stbir__get_coefficient(coefficients, filter, scale_ratio, j, skip) == 0
    )
      skip++;
...
static float *stbir__get_decode_buffer(stbir__info *stbir_info) {
  ...
  return &stbir_info->decode_buffer
            [stbir_info->horizontal_filter_pixel_margin * stbir_info->channels];
}
...
static void stbir__decode_scanline(stbir__info *stbir_info, int n) {
    ...
    for (; x < max_x; x++) {
      ...
      for (c = 0; c < channels; c++)
        decode_buffer[decode_pixel_index + c] =
          ((float)((const unsigned char *)input_data)[input_pixel_index + c]) /
          255;
    }
    break;
  ...
  case STBIR__DECODE(STBIR_TYPE_UINT8, STBIR_COLORSPACE_SRGB):
    for (; x < max_x; x++) {
      ...
      for (c = 0; c < channels; c++)
        decode_buffer[decode_pixel_index + c] =
          stbir__srgb_uchar_to_linear_float[((const unsigned char *)input_data
          )[input_pixel_index + c]];

      if (!(stbir_info->flags & STBIR_FLAG_ALPHA_USES_COLORSPACE))
        decode_buffer[decode_pixel_index + alpha_channel] =
          ((float)((const unsigned char *)input_data
          )[input_pixel_index + alpha_channel]) /
          255;
    }
    break;
  ...
  case STBIR__DECODE(STBIR_TYPE_UINT16, STBIR_COLORSPACE_LINEAR):
    for (; x < max_x; x++) {
      ...
      for (c = 0; c < channels; c++)
        decode_buffer[decode_pixel_index + c] =
          ((float)((const unsigned short *)input_data)[input_pixel_index + c]) /
          65535;
    }
    break;
...
static float *stbir__get_ring_buffer_entry(
  float *ring_buffer,
  int    index,
...
static float *
stbir__add_empty_ring_buffer_entry(stbir__info *stbir_info, int n) {
  ...
  if (stbir_info->ring_buffer_begin_index < 0) {
    ...
    ring_buffer_index = (stbir_info->ring_buffer_begin_index +
                         ...
                         1) %
                        stbir_info->vertical_filter_pixel_width;
    STBIR__DEBUG_ASSERT(
      ring_buffer_index != stbir_info->ring_buffer_begin_index
    );
  }
...
static void stbir__resample_horizontal_upsample(
  stbir__info *stbir_info,
  int          n,
  float       *output_buffer
) {
  ...
  for (x = 0; x < output_w; x++) {
    ...
    case 1:
      for (k = n0; k <= n1; k++) {
        ...
        float coefficient =
          horizontal_coefficients[coefficient_group + coefficient_counter++];
...
static void stbir__resample_horizontal_downsample(
  stbir__info *stbir_info,
  int          n,
  float       *output_buffer
) {
...
static void
stbir__decode_and_resample_upsample(stbir__info *stbir_info, int n) {
...
static void
stbir__decode_and_resample_downsample(stbir__info *stbir_info, int n) {
  // Decode the nth scanline from the source image into the decode buffer.
  stbir__decode_scanline(stbir_info, n);
...
static float *stbir__get_ring_buffer_scanline(
  ...
  int ring_buffer_index =
    (begin_index + (get_scanline - first_scanline)) % ring_buffer_size;
  return stbir__get_ring_buffer_entry(
    ring_buffer,
    ring_buffer_index,
    ring_buffer_length
  );
...
static void stbir__encode_scanline(
  stbir__info *stbir_info,
  int          num_pixels,
  void        *output_buffer,
  float       *encode_buffer,
...
#define STBIR__ENCODE_LINEAR16(f)                                              \
  stbir__saturate16(STBIR__ROUND_INT((f) * 65535))
#else
#define STBIR__ENCODE_LINEAR8(f)                                               \
  (unsigned char)STBIR__ROUND_INT(stbir__saturate(f) * 255)
#define STBIR__ENCODE_LINEAR16(f)                                              \
  (unsigned short)STBIR__ROUND_INT(stbir__saturate(f) * 65535)
#endif
      ...
      for (n = 0; n < num_nonalpha; n++) {
        ...
        ((unsigned short *)output_buffer)[index] =
          (unsigned short)STBIR__ROUND_INT(
            stbir__linear_to_srgb(stbir__saturate(encode_buffer[index])) * 65535
          );
      }

      if (!(stbir_info->flags & STBIR_FLAG_ALPHA_USES_COLORSPACE))
        ((unsigned short *)output_buffer)[pixel_index + alpha_channel] =
          STBIR__ENCODE_LINEAR16(encode_buffer[pixel_index + alpha_channel]);
    }
      ...
      for (n = 0; n < channels; n++) {
        ...
        ((unsigned int *)output_buffer)[index] =
          (unsigned int)STBIR__ROUND_UINT(
            ((double)stbir__saturate(encode_buffer[index])) * 4294967295
          );
      }
    }
    break;
  ...
  case STBIR__DECODE(STBIR_TYPE_UINT32, STBIR_COLORSPACE_SRGB):
    for (x = 0; x < num_pixels; ++x) {
      ...
      for (n = 0; n < num_nonalpha; n++) {
        ...
        ((unsigned int *)output_buffer)[index] =
          (unsigned int)STBIR__ROUND_UINT(
            ((double)stbir__linear_to_srgb(stbir__saturate(encode_buffer[index])
            )) *
            4294967295
          );
      }
      ...
      if (!(stbir_info->flags & STBIR_FLAG_ALPHA_USES_COLORSPACE))
        ((unsigned int *)output_buffer)[pixel_index + alpha_channel] =
          (unsigned int)STBIR__ROUND_INT(
            ((double)stbir__saturate(encode_buffer[pixel_index + alpha_channel])
            ) *
            4294967295
          );
    }
...
static void stbir__resample_vertical_upsample(
  stbir__info *stbir_info,
  int          n,
  ...
  stbir__encode_scanline(
    stbir_info,
    output_w,
    (char *)output_data + output_row_start,
    encode_buffer,
    channels,
    alpha_channel,
    decode
...
static void stbir__buffer_loop_upsample(stbir__info *stbir_info) {
  ...
  for (y = 0; y < stbir_info->output_h; y++) {
    ...
    STBIR__DEBUG_ASSERT(
      in_last_scanline - in_first_scanline <=
      stbir_info->vertical_filter_pixel_width
    );
    ...
    while (in_last_scanline > stbir_info->ring_buffer_last_scanline)
      stbir__decode_and_resample_upsample(
        stbir_info,
        stbir_info->ring_buffer_last_scanline + 1
      );
...
static void stbir__empty_ring_buffer(
  stbir__info *stbir_info,
...
static void stbir__buffer_loop_downsample(stbir__info *stbir_info) {
  ...
  int   output_h    = stbir_info->output_h;
  float in_pixels_radius =
    stbir__filter_info_table[stbir_info->vertical_filter].support(scale_ratio) /
    scale_ratio;
  ...
  for (y = -pixel_margin; y < max_y; y++) {
    ...
    STBIR__DEBUG_ASSERT(
      out_last_scanline - out_first_scanline <=
      stbir_info->vertical_filter_pixel_width
    );
...
static void stbir__calculate_transform(
  stbir__info *info,
  float        s0,
  float        t0,
  float        s1,
...
static void stbir__choose_filter(
  stbir__info *info,
...
static stbir_uint32 stbir__calculate_memory(stbir__info *info) {
  int pixel_margin = stbir__get_filter_pixel_margin(
    info->horizontal_filter,
    info->horizontal_scale
  );
  int filter_height =
  ...
  info->ring_buffer_size =
    info->output_w * info->channels * filter_height * sizeof(float);
  info->encode_buffer_size = info->output_w * info->channels * sizeof(float);

  STBIR_ASSERT(info->horizontal_filter != 0);
  STBIR_ASSERT(
    info->horizontal_filter < STBIR__ARRAY_SIZE(stbir__filter_info_table)
  ); // this now happens too late
  STBIR_ASSERT(info->vertical_filter != 0);
  STBIR_ASSERT(
    info->vertical_filter < STBIR__ARRAY_SIZE(stbir__filter_info_table)
  ); // this now happens too late
...
#define STBIR__NEXT_MEMPTR(current, newtype)                                   \
  ...
  if (stbir__use_height_upsampling(info)) {
    ...
    STBIR__DEBUG_ASSERT(
      (size_t)STBIR__NEXT_MEMPTR(info->ring_buffer, unsigned char) ==
      (size_t)tempmem + tempmem_size_in_bytes
    );
  }
...
#undef STBIR__NEXT_MEMPTR
  ...
  stbir__calculate_filters(
    info,
    info->horizontal_contributors,
    info->horizontal_coefficients,
    info->horizontal_filter,
    info->horizontal_scale,
    info->horizontal_shift,
    info->input_w,
    info->output_w
  );
  stbir__calculate_filters(
  ...
  if (stbir__use_height_upsampling(info)) stbir__buffer_loop_upsample(info);
