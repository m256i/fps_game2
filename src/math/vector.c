...
/* forward decls */
u0 vec3_add_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0, const vec3 *const _v1);
u0 vec3_add_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0, const vec3 *const _v1);
u0 vec3_sub_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0, const vec3 *const _v1);
u0 vec3_sub_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0, const vec3 *const _v1);
u0 vec3_scale_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0, f32 _s);
u0 vec3_scale_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0, f32 _s);
u0 vec3_cross_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0, const vec3 *const _v1);
u0 vec3_cross_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0, const vec3 *const _v1);
...
vec3 vec3_cross_impl_SSE(const vec3 *const _v0, const vec3 *const _v1) {
  __m128 tmp0 = _mm_shuffle_ps(_v0->vec128, _v0->vec128, _MM_SHUFFLE(3, 0, 2, 1));
  __m128 tmp1 = _mm_shuffle_ps(_v1->vec128, _v1->vec128, _MM_SHUFFLE(3, 1, 0, 2));
...
f32 vec3_length_impl_noSIMD(const vec3 *const _v0) { return sqrtf(vec3_dot_impl_noSIMD(_v0, _v0)); }

f32 vec3_length_impl_SSE(const vec3 *const _v0) { return sqrtf(vec3_dot_impl_SSE(_v0, _v0)); }
...
bool vec3_approx_eq_impl_noSIMD(const vec3 *const _v0, const vec3 *const _v1, f32 _tol) {
  return vec3_dist_impl_noSIMD(_v0, _v1) <= _tol;
}

bool vec3_approx_eq_impl_SSE(const vec3 *const _v0, const vec3 *const _v1, f32 _tol) {
...
u0 vec3_add_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0, const vec3 *const _v1) {
  *_res = (vec3){.x = _v0->x + _v1->x, .y = _v0->y + _v1->y, .z = _v0->z + _v1->z};
}
...
u0 init_mathlib(u0) {
  ...
  if (!has_sse) { GAME_WARNF("engine::mathlib: will not use SSE4.2 instructions"); }
  if (!has_avx2) { GAME_WARNF("engine::mathlib: will not use AVX2 instructions"); }
...
static bool f32_equal_impl_noSIMD(f32 *a, f32 *b) { return fabsf(*b - *a) <= 1e-6f; }
static bool bool_equal_impl_noSIMD(bool *a, bool *b) { return *a == *b; }
/*
...
#define DO_TEST_SSE_VEC3_VEC3(_result_type, _op, ...)                                                                  \
  do {                                                                                                                 \
    ...
    if (!_result_type##_approx_eq_impl_noSIMD(&res, &res1, 1e-6f)) {                                                   \
      printf("mathlib::unit warn: (%f, %f,%f) != (%f, %f,%f) operation: '%s' values: '%s' ", res.x, res.y, res.z,      \
             res1.x, res1.y, res1.z, #_op, #__VA_ARGS__);                                                              \
...
#define DO_TEST_SSE_F32_VEC3(_result_type, _op, ...)                                                                   \
  do {                                                                                                                 \
    ...
    if (!_result_type##_equal_impl_noSIMD(&res, &res1)) {                                                              \
      printf("mathlib::unit warn: (%f) != (%f) operation: '%s' values: '%s' delta: %f\n", res, res1, #_op,             \
             #__VA_ARGS__, fabsf(res1 - res));                                                                         \
    }                                                                                                                  \
  } while (0)
...
#define DO_TEST_SSE_BOOL_VEC3(_result_type, _op, ...)                                                                  \
  do {                                                                                                                 \
    ...
    _result_type res1 = _op##_impl_SSE(__VA_ARGS__);                                                                   \
    if (!_result_type##_equal_impl_noSIMD(&res, &res1)) {                                                              \
      printf("mathlib::unit warn: (%u) != (%u) operation: '%s' values: '%s'\n", res, res1, #_op, #__VA_ARGS__);        \
    }                                                                                                                  \
  } while (0)

static f32 generate_random_f32(u0) {
  f32 lol = (f32)rand();
  if (lol <= __FLT_EPSILON__) { lol += __FLT_EPSILON__; }
  return ((f32)rand() / lol);
}

static vec3 generate_random_vec3(u0) {
  return (vec3){.x = generate_random_f32(), .y = generate_random_f32(), .z = generate_random_f32()};
}
