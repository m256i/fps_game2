...
/* forward decls */
u0 vec3_add_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0,
                                const vec3 *const _v1);
u0 vec3_add_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0,
                             const vec3 *const _v1);
u0 vec3_sub_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0,
                                const vec3 *const _v1);
u0 vec3_sub_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0,
                             const vec3 *const _v1);
u0 vec3_scale_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0,
                                  f32 _s);
u0 vec3_scale_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0, f32 _s);
u0 vec3_cross_inplace_impl_noSIMD(vec3 *const _res, const vec3 *const _v0,
                                  const vec3 *const _v1);
u0 vec3_cross_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0,
                               const vec3 *const _v1);
...
vec3 vec3_add_impl_noSIMD(const vec3 *const _v0, const vec3 *const _v1) {
  return (vec3){
      .r = _v0->r + _v1->r, .g = _v0->g + _v1->g, .b = _v0->b + _v1->b};
}
...
bool vec3_approx_eq_impl_noSIMD(const vec3 *const _v0, const vec3 *const _v1,
                                f32 _tol) {
  return vec3_dist_impl_noSIMD(_v0, _v1) <= _tol;
}

bool vec3_approx_eq_impl_SSE(const vec3 *const _v0, const vec3 *const _v1,
                             f32 _tol) {
...
u0 vec3_scale_inplace_impl_SSE(vec3 *const _res, const vec3 *const _v0,
                               f32 _s) {
...
#define DO_TEST_SSE_VEC3_VEC3(_result_type, _op, ...)                          \
  do {                                                                         \
    ...
    if (!_result_type##_approx_eq_impl_noSIMD(&res, &res1, 1e-6f)) {           \
      printf("mathlib::unit warn: (%f, %f,%f) != (%f, %f,%f) operation: '%s' " \
             "values: '%s' ",                                                  \
             res.x, res.y, res.z, res1.x, res1.y, res1.z, #_op, #__VA_ARGS__); \
...
#define DO_TEST_SSE_F32_VEC3(_result_type, _op, ...)                           \
  do {                                                                         \
    ...
    if (!_result_type##_equal_impl_noSIMD(&res, &res1)) {                      \
      printf("mathlib::unit warn: (%f) != (%f) operation: '%s' values: '%s' "  \
             "delta: %f\n",                                                    \
             res, res1, #_op, #__VA_ARGS__, fabsf(res1 - res));                \
    }                                                                          \
  } while (0)
