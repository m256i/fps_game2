...
create_persistent_resource_data(const gl_resource_data *const _temp) {
  ...
  GAME_LOGF(
    "doig deep copy on resource %s %d",
    _temp->resource_name,
    _temp->desc.dummy.creation_info_type
  );
  ...
  case RESOURCE_CREATION_INFO_TYPE_SHADER: {
    ...
    /*
    dont copy all of the read only attributes
    */
    break;
  }
  ...
  GAME_LOGF(
    "out resource: %s %d",
    out.resource_name,
    out.desc.dummy.creation_info_type
  );
  return out;
}
...
u0 destroy_persistent_resource_data(gl_resource_data *_data) {
  if (_data->resource_name) {
    GAME_LOGF(
      ...
      _data->resource_name,
      _data->desc.dummy.creation_info_type
    );
  ...
  case RESOURCE_CREATION_INFO_TYPE_SHADER: {
    ...
    ptr->vertex_path   = NULL;
    ptr->fragment_path = NULL;

    for (usize i = 0; i != ptr->num_inputs; ++i) {
      TRACKED_FREE(ptr->input_attributes[i].name);
    }

    for (usize i = 0; i != ptr->num_ubos; ++i) {
      TRACKED_FREE(ptr->uniform_attributes[i].name);
    }
    ...
    ptr->input_attributes    = NULL;
    ptr->uniform_attributes  = NULL;
    ptr->ubo_binding_points  = NULL;
    ptr->ssbo_binding_points = NULL;
    ptr->output_attachements = NULL;
    break;
  }
  case RESOURCE_CREATION_INFO_TYPE_TEXTURE: {
    break;
...
bool resource_data_eq(
  ...
  case RESOURCE_CREATION_INFO_TYPE_SHADER: {
    ...
    for (usize i = 0; i != a->num_inputs; ++i) {
      ...
      same_desc &=
        a->input_attributes[i].location == b->input_attributes[i].location;
    ...
    for (usize i = 0; i != a->num_uniforms; ++i) {
      ...
      same_desc &=
        a->uniform_attributes[i].location == b->uniform_attributes[i].location;
...
GLuint create_gl_vbo(gl_resource_data *const _resource_data) {
  ...
  for (usize i = 0; i < vc->num_attributes; i++) {
    ...
    GAME_ASSERT(is_valid_vertex_attrib_type(a_info->attribute_type));
    GAME_ASSERT(
      a_info->attribute_count > 0 ||
      a_info->attribute_count < MAX_VERTEX_ATTRIB_COUNT
    );
...
GLuint create_gl_rbo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(is_valid_internal_image_fmt(
    _resource_data->desc.render_buffer.internal_format
  ));
  GAME_ASSERT(
    _resource_data->desc.render_buffer.width > 0 &&
    _resource_data->desc.render_buffer.width <= MAX_IMAGE_SIZEX
  );
  GAME_ASSERT(
    _resource_data->desc.render_buffer.height > 0 &&
    _resource_data->desc.render_buffer.height <= MAX_IMAGE_SIZEY
  );
  GAME_ASSERT(
    _resource_data->desc.render_buffer.sample_count >= 1 &&
    _resource_data->desc.render_buffer.sample_count % 2 == 0 &&
    _resource_data->desc.render_buffer.sample_count <= 16
  );
...
GLuint create_gl_pbo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(
    _resource_data->desc.pixel_buffer.byte_size < MAX_PIXEL_BUFFER_BYTE_SIZE &&
    _resource_data->desc.pixel_buffer.byte_size > 0
  );
  GAME_ASSERT(
    _resource_data->desc.pixel_buffer.usage == GL_STREAM_READ ||
    _resource_data->desc.pixel_buffer.usage == GL_STREAM_COPY ||
    _resource_data->desc.pixel_buffer.usage == GL_STATIC_READ ||
    _resource_data->desc.pixel_buffer.usage == GL_STATIC_COPY ||
    _resource_data->desc.pixel_buffer.usage == GL_DYNAMIC_READ ||
    _resource_data->desc.pixel_buffer.usage == GL_DYNAMIC_COPY
  );
...
GLuint create_gl_image_texture(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(
    it->wrap_mode == GL_CLAMP_TO_EDGE || it->wrap_mode == GL_CLAMP_TO_BORDER ||
    it->wrap_mode == GL_MIRRORED_REPEAT || it->wrap_mode == GL_REPEAT ||
    it->wrap_mode == GL_MIRROR_CLAMP_TO_EDGE
  );
  GAME_ASSERT(it->scale > 0 && it->scale <= 1.f + __FLT_EPSILON__);
...
GLuint create_gl_texture(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(ti->width > 0 && ti->width <= MAX_IMAGE_SIZEX);
  GAME_ASSERT(ti->height > 0 && ti->height <= MAX_IMAGE_SIZEY);
  GAME_ASSERT(
    ti->wrap_mode == GL_CLAMP_TO_EDGE || ti->wrap_mode == GL_CLAMP_TO_BORDER ||
    ti->wrap_mode == GL_MIRRORED_REPEAT || ti->wrap_mode == GL_REPEAT ||
    ti->wrap_mode == GL_MIRROR_CLAMP_TO_EDGE
  );

  const usize xsize = ti->width;
  const usize ysize = ti->height;

  GAME_ASSERT(is_valid_image_fmt(ti->format));
  GAME_ASSERT(is_valid_internal_image_fmt(ti->internal_format));
...
GLuint create_gl_ubo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(_resource_data->desc.ubo.byte_size > 0);
  GAME_ASSERT(
    _resource_data->desc.ubo.usage == GL_STREAM_DRAW ||
    _resource_data->desc.ubo.usage == GL_STATIC_DRAW ||
    _resource_data->desc.ubo.usage == GL_DYNAMIC_DRAW ||
    _resource_data->desc.ubo.usage == GL_STREAM_READ ||
    _resource_data->desc.ubo.usage == GL_STATIC_READ ||
    _resource_data->desc.ubo.usage == GL_DYNAMIC_READ ||
    _resource_data->desc.ubo.usage == GL_STREAM_COPY ||
    _resource_data->desc.ubo.usage == GL_STATIC_COPY ||
    _resource_data->desc.ubo.usage == GL_DYNAMIC_COPY
  );

  GL_CALL(glBufferData(
    GL_UNIFORM_BUFFER,
    _resource_data->desc.ubo.byte_size,
...
GLuint impl_create_gl_resource(gl_resource_data *const resource_data) {
  ...
  case RESOURCE_CREATION_INFO_TYPE_SHADER: {
    GAME_LOGF("creating Shader");
    return create_gl_shader(resource_data);
  }
  case RESOURCE_CREATION_INFO_TYPE_SHADER_STORAGE_BUFFER: {
...
u0 impl_destroy_gl_resource(
  ...
  case RESOURCE_CREATION_INFO_TYPE_SHADER: {
    GAME_LOGF("destroying Shader");
    destroy_gl_shader(resource_data, _handle);
    break;
  }
  case RESOURCE_CREATION_INFO_TYPE_SHADER_STORAGE_BUFFER: {
...
u0 request_gl_resource(
  ...
  if (!gl_resource_manager.initialized) {
    ...
    gl_resource_manager.supports_bindless = supports_bindless_textures();
