...
create_persistent_resource_data(const gl_resource_data *const _temp) {
  ...
  GAME_LOGF("doig deep copy on resource %s %d", _temp->resource_name,
  ...
  GAME_LOGF("out resource: %s %d", out.resource_name,
...
u0 destroy_persistent_resource_data(gl_resource_data *_data) {
  if (_data->resource_name) {
    GAME_LOGF("freeing resource: %s type %d", _data->resource_name,
...
GLuint create_gl_vbo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(vc->buffer_usage == GL_DYNAMIC_DRAW ||
              vc->buffer_usage == GL_STATIC_DRAW ||
  ...
  for (usize i = 0; i < vc->num_attributes; i++) {
    ...
    GAME_ASSERT(a_info->attribute_count > 0 ||
...
GLuint create_gl_rbo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(_resource_data->desc.render_buffer.width > 0 &&
              _resource_data->desc.render_buffer.width <= MAX_IMAGE_SIZEX);
  GAME_ASSERT(_resource_data->desc.render_buffer.height > 0 &&
              _resource_data->desc.render_buffer.height <= MAX_IMAGE_SIZEY);
  GAME_ASSERT(_resource_data->desc.render_buffer.sample_count >= 1 &&
...
GLuint create_gl_pbo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(_resource_data->desc.pixel_buffer.byte_size <
                  MAX_PIXEL_BUFFER_BYTE_SIZE &&
              _resource_data->desc.pixel_buffer.byte_size > 0);
  GAME_ASSERT(_resource_data->desc.pixel_buffer.usage == GL_STREAM_READ ||
...
GLuint create_gl_image_texture(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(it->wrap_mode == GL_CLAMP_TO_EDGE ||
              it->wrap_mode == GL_CLAMP_TO_BORDER ||
              it->wrap_mode == GL_MIRRORED_REPEAT ||
              it->wrap_mode == GL_REPEAT ||
...
GLuint create_gl_texture(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(ti->wrap_mode == GL_CLAMP_TO_EDGE ||
              ti->wrap_mode == GL_CLAMP_TO_BORDER ||
              ti->wrap_mode == GL_MIRRORED_REPEAT ||
              ti->wrap_mode == GL_REPEAT ||
...
GLuint create_gl_ssbo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(_resource_data->desc.ssbo.usage == GL_STREAM_DRAW ||
...
GLuint create_gl_ubo(gl_resource_data *const _resource_data) {
  ...
  GAME_ASSERT(_resource_data->desc.ubo.usage == GL_STREAM_DRAW ||
  ...
  GL_CALL(glBufferData(GL_UNIFORM_BUFFER, _resource_data->desc.ubo.byte_size,
...
u0 request_gl_resource(gl_resource_data *const resource_data,
  ...
  *_handle = new_handle;
  ...
  GAME_LOGF("inserted resource table slot: %p",
